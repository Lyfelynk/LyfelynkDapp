...
const NutritionTracker = () => {
  ...
  useEffect(() => {
    setMeals(Array(numMeals).fill().map(() => []));
  }, [numMeals]);
  ...
  const stringMatchSearch = (query) => {
    ...
    return data.filter(item => item.name.toLowerCase().includes(query.toLowerCase())).slice(0, 5);
  };

  const calculateNutrients = (items) => {
    const totalNutrients = {
      calories: 0, carbohydrates_total_g: 0, protein_g: 0, fat_total_g: 0,
      fat_saturated_g: 0, sodium_mg: 0, potassium_mg: 0, cholesterol_mg: 0,
      fiber_g: 0, sugar_g: 0
    };
    ...
    items.forEach(({ item, quantity, isUnit }) => {
      ...
      if (foodItem) {
        const quantityInGrams = isUnit ? quantity * (unitBasedItems[item.toLowerCase()] || 100) : quantity;
    ...
    return Object.fromEntries(
      Object.entries(totalNutrients).map(([key, value]) => [key, Number(value.toFixed(2))])
    );
  };
  ...
  const handleAddItem = (mealIndex) => {
    setMeals(prevMeals => {
      ...
      newMeals[mealIndex] = [...newMeals[mealIndex], { item: '', quantity: 0, isUnit: false }];
      return newMeals;
    });
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Food Calorie and Nutrient Tracker</h1>
          ...
          <CardContent>
            {meal.map((item, itemIndex) => (
              <div key={itemIndex} className="flex space-x-2 mb-2">
                <Select
                  onValueChange={(value) => handleItemChange(mealIndex, itemIndex, 'item', value)}
                  value={item.item}
                >
                ...
                <Input
                  type="number"
                  value={item.quantity}
                  onChange={(e) => handleItemChange(mealIndex, itemIndex, 'quantity', Number(e.target.value))}
                  placeholder="Quantity"
                />
                <Select
                  onValueChange={(value) => handleItemChange(mealIndex, itemIndex, 'isUnit', value === 'units')}
      ...
      <Button onClick={handleCalculate} className="mb-4">Calculate Nutrients</Button>
